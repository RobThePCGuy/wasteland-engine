...
travelRouter.post('/', async (req: any, res: any) => {
  ...
  if (isEnabled('structured_generation')) {
    ...
    try {
      ...
      // 'repair_needed' means the spec PASSED hard validation but has repairable imperfections:
      // usable at runtime, but NOT canon-eligible (canon_promotion is a separate, off-by-default
      // flag). Make that distinction explicit rather than letting it fall through as a clean admit.
      const canonEligible = result.admission.decision === 'admit';
