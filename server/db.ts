...
export function initDb() {
  db.exec(`
    ...
    -- Indexes on the foreign-key columns the hot-path queries filter on (state load, combat, loot).
    ...
    CREATE INDEX IF NOT EXISTS idx_npc_items_npc ON npc_items(npc_id);
    CREATE INDEX IF NOT EXISTS idx_narrative_log_player ON narrative_log(player_id);
    CREATE INDEX IF NOT EXISTS idx_combat_players_player ON combat_players(player_id);
    CREATE INDEX IF NOT EXISTS idx_combat_npcs_combat ON combat_npcs(combat_id);
    CREATE INDEX IF NOT EXISTS idx_travel_requests_player ON travel_requests(player_id);
    CREATE INDEX IF NOT EXISTS idx_world_deltas_target ON world_deltas(target_type, target_id);
  `);
  ...
  // Ensure scavenge resource items exist. Idempotent (items.name is UNIQUE) so this also
  // backfills pre-existing databases where 'water'/'tech' scavenging previously awarded nothing.
  db.prepare(`
    INSERT OR IGNORE INTO items (name, description, type, weight, value, effects, stackable)
    VALUES
    ('Purified Water', 'Clean, drinkable water. A precious commodity in the wasteland.', 'junk', 0.5, 20, '{}', 1),
    ('Energy Cell', 'A compact fusion cell. Powers energy weapons and devices.', 'junk', 0.1, 25, '{}', 1)
  `).run();
