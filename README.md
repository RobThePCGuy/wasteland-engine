...
Each flag's default can now be set from the environment (e.g. `STRUCTURED_GENERATION=true`), so you can enable a stage without editing code — but every flag still ships off by default, and the legacy seed path is what runs out of the box.
...
> Note: `canon_promotion` has no approval workflow yet (no pending-state queue or review endpoint), so leave it off until that's built. `structured_generation` does not depend on it.
...
Frontend is React 19 with Tailwind. Backend is Express and SQLite via better-sqlite3. Single process.

## Security & Production

This started as a local-only prototype and has since been hardened for public, self-hosted deployment. What that hardening covers:

- **The Gemini key stays on the server.** All Gemini calls are proxied through authenticated routes under `/api/ai/*` (`server/routes/ai.ts`). The key is never inlined into the client bundle — the old Vite `define` that shipped it to the browser is gone.
- **Auth secret is enforced.** The server refuses to start in production if `JWT_SECRET` is unset, instead of silently signing tokens with a default.
- **Server-authoritative outcomes.** Scavenge amounts, trade quantities, loot, and combat targeting are validated and resolved server-side; untrusted request bodies are checked (`zod`) before they touch state. Multi-row mutations run inside SQLite transactions.
- **Abuse limits.** `express-rate-limit` guards auth (strict) and the API surface (global). FK indexes and log pruning keep the single-process DB honest under load.
- **Production serving.** `npm run build` produces both a client bundle and a bundled server (`dist/server.cjs`); with `NODE_ENV=production` the server serves the static client with an SPA fallback.

If you deploy behind a reverse proxy or CDN, set `app.set('trust proxy', <hops>)` in `server.ts` so the rate limiter keys on the real client IP rather than the proxy.

**AI model lifecycle:** the image model `gemini-2.5-flash-image` (sprites/portraits) is scheduled to shut down 2026-10-02 — migrate to `gemini-3.1-flash-image-preview`. The text model `gemini-3-flash-preview` is a preview superseded by GA `gemini-3.5-flash`. Both model ids are centralized at the top of `server/routes/ai.ts` for a one-line swap.
...
Enabling the structured generation pipeline end-to-end (`STRUCTURED_GENERATION=true`) is wired but unverified against a live key — the `generateLocationSpec` step makes real Gemini calls, so run a travel→generate→admit→realize→persist smoke test against a scratch DB before turning it on in production. The pure-function layers (admission, repair, realization) are already unit-tested.
...
You'll need a [Google AI Studio](https://aistudio.google.com/) API key with access to `gemini-3-flash-preview`. `JWT_SECRET` is **required in production** (the server won't start without it) and any long random string works (e.g. `openssl rand -base64 48`).
...
**SQLite note:** This project uses better-sqlite3 v12.9.0, which bundles a current SQLite well above the 3.50.2 threshold for CVE-2025-6965, so that vulnerability doesn't apply here.

**Other commands:**
`npm run build` produces the client + server build, `npm run lint` runs TypeScript type checking, and `npm run test` runs the test suite.
