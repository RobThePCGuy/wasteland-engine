...
export default function IsometricExplorationView({
  ...
  const actors = useMemo(() => {
    ...
    if (player?.position) {
      const equippedWeapon = inventory.find(i => i.id === player.equipment?.weapon_id);
      const equippedArmor = inventory.find(i => i.id === player.equipment?.armor_id);
      result.push({
        id: 'player',
        name: player.name,
        x: player.position.x,
        ...
        equipment: equippedWeapon?.name || equippedArmor?.name || '',
      });
    }
