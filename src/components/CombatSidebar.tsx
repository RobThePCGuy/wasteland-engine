...
interface CombatSidebarProps {
  ...
  gameState: GameState;
  onAction: (actionType: string, targetId?: number, bodyPart?: string, useCritical?: boolean) => void | Promise<void>;
...
export default function CombatSidebar({
  ...
  const canCrit = criticalMeter >= 100;
          ...
          <div>
              ...
              {combatState.npcs.map((npc: Combatant) => (
                <div key={npc.id} className="border border-red-900 p-2 mb-2">
                  ...
                  {pendingAction && (
                    <button
                      onClick={() => {
                        onAction(pendingAction, npc.id, selectedBodyPart || undefined, useCritical);
            ...
            {currentTurn?.type === 'player' && !pendingAction && (
              <div className="mt-auto">
                <div className="text-sm mb-2 text-red-400 flex justify-between">
                  <span>AP: {currentTurn?.ap_remaining}</span>
                  <span>CRIT: {criticalMeter}%</span>
                </div>
                {!isGenerating && (
              ...
              <div className="mt-auto">
                {canCrit && (
                  <button
                    ...
                    className={`w-full border p-1 text-xs mb-2 ${useCritical ? 'bg-red-500 text-black border-red-500' : 'border-red-500 text-red-500 hover:bg-red-900'}`}
                  >
                    {useCritical ? 'CRITICAL STRIKE ACTIVE' : 'USE CRITICAL STRIKE'}
                  </button>
                )}
                <button
                  onClick={() => {
            ...
            {pendingAction === 'attack' && (
              <div className="mt-auto">
                ...
                <button onClick={() => { setPendingAction(null); setUseCritical(false); }} className="w-full border border-gray-500 p-1 text-xs hover:bg-gray-800 text-gray-400">CANCEL ATTACK</button>
              </div>
            )}
          </div>
        </div>
