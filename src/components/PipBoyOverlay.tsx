...
function InventoryTab({ inventory, equipment, onUseItem, onEquipItem, onUnequipItem, onDropItem, onReloadItem }: {
  inventory: any[];
  equipment: any;
  ...
  return (
    <div className="flex flex-col gap-3">
      {inventory.map((item: any) => {
        const isEquipped = equipment?.weapon_id === item.id || equipment?.armor_id === item.id;
        return (
          <div key={item.id} className="border border-green-900 p-4 flex justify-between items-center hover:bg-green-900/10 transition-all group">
            ...
            <div className="flex-1">
              <div className="font-bold flex items-center gap-2">
                ...
                {isEquipped && (
                  <span className="text-[8px] bg-green-500 text-black px-1 font-bold">
                    EQUIPPED {item.type === 'weapon' && item.effects && item.effects.includes('"ammo_type"') ? `(${equipment?.ammo_in_clip || 0} AMMO)` : ''}
                  </span>
                )}
              </div>
            ...
            <div className="flex gap-2">
              ...
              {(item.type === 'weapon' || item.type === 'armor') && (
                isEquipped ? (
                  <>
                    <button onClick={() => onUnequipItem(item.id)} className="border border-green-500 px-4 py-1 text-xs hover:bg-green-500 hover:text-black transition-all font-bold">UNEQUIP</button>
                    {item.type === 'weapon' && item.effects.includes('"ammo_type"') && onReloadItem && (
                      <button onClick={() => onReloadItem(item.id)} className="border border-green-500 px-4 py-1 text-xs hover:bg-green-500 hover:text-black transition-all font-bold">RELOAD</button>
                    )}
                  </>
                ) : (
              ...
              <button onClick={() => onDropItem(item.id)} className="border border-red-900 text-red-500 px-4 py-1 text-xs hover:bg-red-900 hover:text-white transition-all font-bold">DROP</button>
            </div>
          </div>
        );
      })}
