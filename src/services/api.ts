...
export const api = {
  ...
  items: {
    ...
    async unequip(token: string | null, itemId: number) {
      return request<{ message: string }>('/api/item/' + itemId + '/unequip', {
        method: 'POST',
        headers: authHeaders(token),
      });
    },

    async drop(token: string | null, itemId: number) {
      return request<{ message: string }>('/api/item/' + itemId + '/drop', {
        method: 'POST',
        headers: authHeaders(token),
      });
    },
  ...
  interact: {
    ...
    async rest(token: string | null) {
      return request<{ message: string; healed: number }>('/api/interact/rest', {
        method: 'POST',
        headers: authHeaders(token),
      });
    },
