...
export default function App() {
      ...
      if (stateRes.ok && worldRes.ok && locationsRes.ok && perksRes.ok) {
        ...
        if (data.location?.npcs) {
          data.location.npcs.forEach((npc: any) => {
            if (npc.hit_points > 0) gemini.generateCharacterSprite(npc.name, 'A wasteland wanderer.', npc.is_hostile);
          });
        }
        if (data.inventory) {
          data.inventory.forEach((item: any) => gemini.generateItemIcon(item.name, item.type, item.description));
        }
  ...
  const handleRest = async () => {
    try {
      const { ok, data } = await api.interact.rest(token);
      if (!ok) { addNarrative(`Rest failed: ${(data as any).message}`, 'system'); return; }
      addNarrative((data as any).message, 'system'); await loadGame();
    } catch (error) { handleApiError(error); }
  };
  ...
  const handleUnequipItem = async (itemId: number) => {
    try {
      const { ok, data } = await api.items.unequip(token, itemId);
      if (!ok) return;
      addNarrative((data as any).message, 'system'); await loadGame();
    } catch (error) { handleApiError(error); }
  };
  ...
  const handleDropItem = async (itemId: number) => {
    try {
      const { ok, data } = await api.items.drop(token, itemId);
      if (!ok) return;
      addNarrative((data as any).message, 'system'); await loadGame();
    } catch (error) { handleApiError(error); }
  };

  const handleReloadItem = async (itemId: number) => {
    try {
      const res = await fetch(`/api/items/${itemId}/reload`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      addNarrative(data.message, 'system');
      await loadGame();
    } catch (error: any) {
      addNarrative(error.message, 'system');
    }
  };
      ...
      <div className="crt h-screen bg-black text-green-500 font-mono p-4 flex flex-col">
        ...
        {pipBoyTab && (
          <PipBoyOverlay
            ...
            onUseItem={handleUseItem} onEquipItem={handleEquipItem} onUnequipItem={handleUnequipItem} onDropItem={handleDropItem} onReloadItem={handleReloadItem}
            ...
            <div className="h-full flex flex-col gap-4 overflow-y-auto pr-2">
              {combatState && gameState ? (
                ...
                <ExplorationSidebar gameState={gameState} quest={quest} loadingQuest={loadingQuest} loadingTravel={loadingTravel}
                  ...
                  onLoot={handleLoot} onRest={handleRest} onOpenEditor={() => setShowEditor(true)} />
              )}
            </div>
          </Panel>
          ...
          <Panel defaultSize={45} minSize={20} className="flex flex-col h-full">
            <div className="h-full flex flex-col overflow-hidden">
              {combatState && gameState ? (
                ...
                <IsometricExplorationView location={gameState?.location ?? null} player={gameState?.player ?? null} inventory={gameState?.inventory ?? []}
